...
t.test('task tags include @from_rtm after existing tags', async t => {
  ...
  const tmpJson = path.join(__dirname, '..', 'data', `rtm_test_task_from_rtm_${stamp}.json`);
  const outCsv = path.join(__dirname, '..', 'data', `output_test_from_rtm_${stamp}.csv`);
  const outJson = path.join(__dirname, '..', 'data', `used_records_test_from_rtm_${stamp}.json`);
  ...
  const testData = {
    ...
    tasks: [ { id: 'T1', series_id: 'S1', title: 'Test task', content: 'Do the thing', tags: ['alpha','beta'], list_id: 'L1' } ],
    notes: []
  };
  fs.writeFileSync(tmpJson, JSON.stringify(testData, null, 2), 'utf8');

  const cmd = 'node';
  const args = [path.join(__dirname, '..', 'transform.js'), '--input', tmpJson, '--csv', sampleCsv, '--rowsPath', 'tasks', '--output', outCsv, '--output-json', outJson];
  const res = child_process.spawnSync(cmd, args, { encoding: 'utf8' });
  t.equal(res.status, 0, `transform.js exited with status ${res.status}; stderr=${res.stderr}`);
  ...
  for (let i = 1; i < records.length; i++) {
    ...
    if (type === 'task') {
      ...
      t.match(content, /@alpha/, 'should include @alpha');
      t.match(content, /@beta/, 'should include @beta');
      t.match(content, /@from_rtm/, 'should include @from_rtm');
      const posFrom = content.indexOf('@from_rtm');
      const posAlpha = content.indexOf('@alpha');
      t.ok(posFrom > posAlpha, '@from_rtm should come after existing tags');
      found = true;
      break;
    }
  }
